...
inline void layout_row(std::vector<Node>& nodes, const std::vector<double>& areas,
    ...
    const float thickness = float(sum / double(side));
    const bool isHoriz = r.w < r.h;
    float offset = isHoriz ? r.x : r.y;

    for (size_t i : row) {
        const float itemLen = float(areas[i] / sum * double(side));
        nodes[i].rect =
            isHoriz ? Rect{offset, r.y, itemLen, thickness} : Rect{r.x, offset, thickness, itemLen};
        offset += itemLen;
    }
    if (isHoriz) {
        r.y += thickness;
...
inline void squarify_level(std::vector<Node>& nodes, Rect rect) {
    ...
    for (size_t i = 0; i < areas.size(); ++i) {
        ...
        nextRow.push_back(i);
        if (row.empty() ||
            worst_aspect(areas, nextRow, rowSum + areas[i], side) <=
