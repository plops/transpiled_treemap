// Extension palette + deterministic hash fallback. Mirrors the Rust
...
inline Color color_for_path(const fs::path& path) {
    ...
    if (ext == ".rs" || ext == ".c" || ext == ".cpp" || ext == ".py" || ext == ".js" ||
        ext == ".ts" || ext == ".txt" || ext == ".md") {
    ...
    if (ext == ".mp4" || ext == ".mkv" || ext == ".mov" || ext == ".mp3" ||
        ext == ".flac") {
