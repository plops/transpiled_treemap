// Treemap Disk Visualizer: serial reference + parallel variant (plan 20260919_01_merge).
//
// Serial path mirrors 00_mvp (macroquad, no text in treemap). Parallel path
// uses only std::thread::scope over owned values (no new dependency).
// `--scan <dir>` prints one line (BrokenPipe-safe). `--bench <dir>` prints
// serial vs parallel scan/layout timings. Otherwise the GUI runs (parallel).
...
fn scan_tree_parallel_impl(path: &Path, depth: usize) -> Node {
    ...
    // get their own scope thread. Thread-per-file was 12x slower (bench).
...
// BrokenPipe-safe single-line printer (02-review lesson).
...
fn window_conf() -> Conf {
    Conf {
        window_title: "Treemap Disk Visualizer (parallel)".to_string(),
        window_width: 1280,
        window_height: 720,
        ..Default::default()
    }
...
// via Window::from_config, like #[macroquad::main] expands to.
fn main() {
...
mod tests {
    ...
    fn serial_parallel_scan_equivalence() {
        let base = fixture("treemap_par_eq");
    ...
    fn serial_parallel_layout_invariants() {
        let base = fixture("treemap_par_layout");
    ...
    fn scan_bench_compare() {
        let base = fixture("treemap_par_bench");
        let t0 = Instant::now();
        let s = scan_tree(&base);
