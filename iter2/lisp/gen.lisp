;; gen.lisp --- cl-cpp-generator2 source of truth for the iter2 C++ core.
;;
;; Run from iter2/scripts/setup06_transpile.sh (or by hand):
;;   sbcl --load iter2/lisp/gen.lisp --quit
;; Writes:
;;   iter2/lisp/gen/010_scanner.hpp  iter2/lisp/gen/015_color.hpp
;;   iter2/lisp/gen/020_layout.hpp   (030_pge_app.hpp stays handwritten: PGE3 only)
;; setup06 copies lisp/gen/*.hpp to cpp/gen/ for the USE_GENERATED build.
;;
...
;; Backup rule: cp lisp/gen.lisp lisp/gen.lisp.known-good before editing.
;; iter2 changes vs iter1/cpp/gen.lisp: single-stat d_type scan
;; (is_symlink/is_directory/is_regular_file, no status() calls), entry paths
;; by value, capped thread fan-out with depth parameter, top-level-only
;; layout fan-out with depth parameter.
(eval-when (:compile-toplevel :execute :load-toplevel)
  (ql:quickload "cl-cpp-generator2"))
(defpackage #:treemap-iter2-gen
  (:use #:cl #:cl-cpp-generator2))
(in-package #:treemap-iter2-gen)
...
(defun emit-types ()
  `(do0
    ...
    (defstruct0 Node
      ...
      (rect Rect)
      (color Color))))
;; Serial scan emitters (each <= 60 lines).
  ...
  `(do0
    (defun scan_entry (node entry)
      ...
      (let ((p (dot entry (path))))
        (declare (type "const fs::path" p))
        (when (dot entry (is_directory ec))
          (progn
            (when ec
              (progn
                (report_skip (string "file_type") p ec)
                (return)))
        ...
        (when ec
          (progn
            (report_skip (string "file_type") p ec)
            (return)))
        (when (not (dot entry (is_regular_file ec)))
          (return))
        (when ec
          (progn
            (report_skip (string "file_type") p ec)
    ...
    (defun scan_tree (path)
      ...
      (let ((node (Node)))
        ...
        "}"
        (return node)))))
;; Parallel scan: files inline, one thread per subdirectory (owned).
;; Fan-out capped via ScanWorkersActive, depth parameter from the start.
(defun emit-scan-parallel ()
  `(do0
    ...
    (defun scan_tree_parallel_impl (path depth)
      (declare (type "const fs::path&" path)
               (type int depth)
               (values Node))
      (let ((node (Node))
        ...
        (let ((entry (deref it)))
          ...
          (let ((p (dot entry (path))))
            (declare (type "const fs::path" p))
            (when (dot entry (is_directory ec2))
              (progn
                (when ec2
                  (progn
                    (report_skip (string "file_type") p ec2)
                    "continue;"))
            ...
            (when ec2
              (progn
                (report_skip (string "file_type") p ec2)
                "continue;"))
            (when (not (dot entry (is_regular_file ec2)))
              "continue;")
            (when ec2
              (progn
                (report_skip (string "file_type") p ec2)
        ...
        (foreach (child results)
          (when (< 0 (dot child size))
            ...
            (dot node children (push_back (std--move child)))))
        (return node)))))
;; Layout emitters: serial rows + top-level-only parallel driver.
(defun emit-layout ()
  `(do0
    ...
    (defun layout_row (nodes areas row sum r)
      ...
      (let ((side (std--min (dot r w) (dot r h)))
            (thickness (cast float (/ sum (cast double side))))
            (isHoriz (< (dot r w) (dot r h)))
            (offset (? isHoriz (dot r x) (dot r y)))))
      (foreach (i row)
        (let ((itemLen (cast float (* (/ (aref areas i) sum)
                                      (cast double side)))))
          (if isHoriz
    ...
    (defun layout_subtree_parallel (node out depth)
      (declare (type Node node)
               (type "Node&" out)
               (type int depth)
               (values void)
      ...
      (when (needs_recursion node)
        (squarify_parallel_depth (dot node children) (dot node rect) (+ depth 1)))
      (= out (std--move node)))
    ...
    (defun squarify_parallel_depth (nodes rect depth)
      (declare (type "std::vector<Node>&" nodes)
               (type Rect rect)
               (type int depth)
               (values void))
...
(progn
  ...
  (write-source "010_scanner.hpp"
                `(do0
                  "#pragma once"
                  (include<> algorithm atomic cstdint filesystem iostream mutex string
                             thread vector)
                  (namespace treemap
