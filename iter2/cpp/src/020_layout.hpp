...
// parallel driver. Parallelism mirrors the iter2 Rust prototype: serial row
// pass, then one thread per owned subdirectory subtree; size order restored
// after join because threads rejoin out of order.
#pragma once
...
// than they saved on large trees (iter1 bench: 5x slower than serial).
inline void squarify_parallel_depth(std::vector<Node>& nodes, Rect rect, int depth)
{
    squarify_level(nodes, rect);
    if (depth > 0)
