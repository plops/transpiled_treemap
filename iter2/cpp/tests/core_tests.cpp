// Equivalence + edge tests for the iter2 C++ parallel core (no PGE needed).
#include <cassert>
#include <filesystem>
#include <fstream>
#include <iostream>
...
fs::path fixture()
{
    const fs::path base = fs::temp_directory_path() / "treemap_iter2_cpp_eq";
    fs::remove_all(base);
...
int main()
{
    ...
    // Regression from iter1: root.rect was never set, so every hover missed.
    {
        treemap::Node root;
